...
1. **Klasy CSS (podstawowy, natychmiastowy)** — wstrzykiwany `<style>` ukrywa wiersze DetailsGroupItems po ich stałych klasach: `genresGroup`, `directorsGroup`, `writersGroup`, `studiosGroup`, `tagsGroup`. Te klasy są stabilne między wersjami jellyfin-web (używa ich cały ekosystem modów).
2. **Etykiety (zapasowy)** — MutationObserver dopasowuje elementy po widocznym tekście etykiety i ukrywa cały wiersz lub sekcję. To on ukrywa sekcję „Do obejrzenia" (Next Up), która nie ma dedykowanej stałej klasy.

## Konfiguracja

Na górze `hide-meta.js` są dwie listy: `HIDE_CLASSES` (klasy wierszy) i `HIDE_LABELS` (etykiety, PL + EN). Obie można edytować bez znajomości reszty kodu.

## Faza działania

**POST** — `post.cmd` kopiuje `hide-meta.js` do katalogu z `index.html` (`www\`) i wstrzykuje `<script>` na końcu `<head>`. Operacja idempotentna i weryfikowana.
